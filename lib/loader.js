...
function emitSprite(name, buffer) {
    let url = loaderUtils.interpolateName(this, 'spirte.[hash:7].png', {
        content: buffer
    });
    ...
    const spritePath = path.resolve(this.context, cssImagePath + url);
    const spirteDirPath = path.resolve(this.context, cssImagePath);

    fse.ensureDirSync(spirteDirPath);
